...
![継承ウマ娘ごとに白因子の面数と内訳を横並びで確認する比較画面](github-assets/compare-white-factors-v0111.png)
...
- `UmaFactor-win-x64-portable-v0.1.11.zip`
...
- 画面上部の一括保存・読込・CSV出力を「全因子データ」と表記し、緑色の背景を外しました。
- 因子タブを開いている間だけ表示する操作欄を「因子データ」と表記しました。
- 因子タブへ「コメント（任意）」を再表示しました。保存したコメントは保存済み因子一覧と比較表の因子データ名へ反映されます。
